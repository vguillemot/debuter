...
![](img/S05ggplot2/data-viz.png){width=50%}

## Un échantillon de ce qu'il est possible de faire avec ggplot2 {.center}

![](img/S05ggplot2/sample.png){width=100%}
...
![](img/S05ggplot2/ggplot-1.png){width=100%}


## Les données {.center}

![](img/S05ggplot2/ggplot-2.png){width=100%}

## Les paramètres esthétiques {.center}

![](img/S05ggplot2/ggplot-3.png){width=100%}

## Les géométries {.center}

![](img/S05ggplot2/ggplot-4.png){width=100%}

## L'image à retenir {.center}

![](img/S05ggplot2/ggplot2_masterpiece.jpg){width=80%}

## [G]rammar of [G]raphics {.center}

![](img/S05ggplot2/grammar-of-graphics.png){width=100%}
...
## A vous ! {.columns-2}
 ...
 1. Comment ajouter une colonne `SucresSup10` : `TRUE` si la teneur en sucres est supérieure à 10, `FALSE` sinon ?
 2. Comment corriger la commande suivante pour obtenir le graphe à droite ?
...
![](img/S05ggplot2/barplotSucres.png){width=100%}
...
![](img/S05ggplot2/wickham.png){width=50%}
...
data.frame(
  randx = randx,
  randx_cut = cut(randx, -3:3))
```

## Avec geom_bar

On a déjà vu comment faire un diagramme en bâtons. Analysons ensemble l'exemple suivant : 

```{r geom_bar_ter, fig.height = 3}
fruits %>%
  mutate(EauCat = cut(Eau, c(0, 84.2, 100))) %>%
  ggplot(aes(EauCat)) + 
  geom_bar(fill = "steelblue")
```

## Intermède : opérations sur les booléens

Les booléens sont transformés en 0s ou 1s quand ils sont utilisés dans des opérations arithmétiques : 

  * `TRUE + 0` est égal à `1`, `TRUE` est traité comme la valeur "1"
  * `FALSE + 0` est égal à `0`, `FALSE` est traité comme la valeur "0"
  
Cela va même plus loin : 

  * `TRUE + TRUE` est égal à "2",
  * `TRUE + FALSE + TRUE + FALSE + FALSE + TRUE` est égal à ?
...
Faites un diagramme en bâtons de la teneur en eau :

  1. Transformez la variable `Eau` en variable catégorielle (avec 4 catégories),
  2. Faites un diagramme en bâtons de cette nouvelle variable.
...
![](img/S05ggplot2/message.png){width=80%}
...
![](img/S05ggplot2/boxplot.png){width=100%}
...
![](img/S05ggplot2/violonfibres.png){width=100%}
...
![](img/S05ggplot2/theme.png){width=100%}
...
ggplot(fruits,
       aes(x = Sucres, 
           ...
           *** = ***)) + 
  geom_***() + 
  ***(title = "Fruits (source : https://ciqual.anses.fr/)",
     x = "Sucres (g/100 g)", 
...
![](img/S05ggplot2/latotale.png){width=100%}

## Au secours, mes points se superposent! {.columns-2}

Pas de panique, on peut utiliser l'opacité (aka `alpha`) : 
...
![](img/S05ggplot2/scalelog.png){width=100%}
...
![](img/S05ggplot2/cheatsheet.png){width=50%}
